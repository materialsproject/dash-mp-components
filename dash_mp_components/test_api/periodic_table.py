...
class PeriodicTable(BasePage):
    ...
    def checkDetailed(self, number, symbol, name):
        table = self.dash_duo.find_element(self.get_table_selector())
        detailed_element = table.find_element_by_css_selector(
            Selectors.detailed.value)
        assert detailed_element.find_element_by_css_selector(
            Selectors.number.value).text == number
        assert detailed_element.find_element_by_css_selector(
            Selectors.symbol.value).text == symbol
        assert detailed_element.find_element_by_css_selector(
            Selectors.name.value).text == name

    def check_if_element_has_class(self, element_symbol, klass):
        assert (klass in self.find_element(element_symbol).get_attribute(
            'class').split()) is True

    def hover_over_element(self, symbol):
        element_to_hover_over = self.find_element(symbol)
        hover = ActionChains(
            self.dash_duo.driver).move_to_element(element_to_hover_over)
        hover.perform()

    def wait_for_table(self):
        self.dash_duo.wait_for_element_by_css_selector(self.base_selector)
        self.dash_duo.wait_for_element_by_css_selector(
            self.get_table_selector())

    # This will throw if the element is not selectable
    def select_element(self, symbol):
        element = self.find_element(symbol)
        element.click()
        wait_for_element_having_class(self.dash_duo, element,
                                      Selectors.enabled.value[1:])
    ...
    def check_if_element_is_hidden(self, symbol):
        ...
        assert hidden_element.find_element_by_css_selector(
            Selectors.number.value).is_displayed() is False
        assert hidden_element.find_element_by_css_selector(
            Selectors.symbol.value).is_displayed() is False
    ...
    def check_table_layout(self, expected_layout):
        ...
        for layout in layouts:
            assert (layout in table.get_attribute('class').split()) is (
